...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        ++con_count;
        pthread_mutex_unlock(&mutex);
        struct con_arg arg = { .p_mutex = &mutex, .p_socket = &ssock,.p_con_count = &con_count };
...
void look_exp_member(int (*condition_f)(unsigned char), unsigned char ch, char** exp_member_str, int *exp_mem_len, int *symbc, int *wphase) {
    if ((*condition_f)(ch)) {
        *symbc = 1;
        *exp_mem_len = EMLEN;
...
void read_exp_member(int (*condition_f)(unsigned char), unsigned char ch, char** exp_member_str, int *exp_mem_len, int *symbc, int *wphase, int wphase_val, int *iter) {
    ++*symbc;
        ...
        for (int i = 0; i < rc; ++i)
        {
            ...
            case 0:
                ...
                if (exp_members[0] == NULL) {
                    look_exp_member(is_number, ch, &exp_members[0], &exp_mem_len, &symbc, &wphase);
                // it's operation case
                } else if (exp_members[2] == NULL) {
                    look_exp_member(is_operation, ch, &exp_members[2], &exp_mem_len, &symbc, &wphase);
                // it's second number case
                } else {
                    look_exp_member(is_number, ch, &exp_members[1], &exp_mem_len, &symbc, &wphase);
                }
                break;
            ...
            case 1:
                ...
                if (exp_members[1] == NULL && exp_members[2] == NULL) {
                    read_exp_member(is_digit, ch, &exp_members[0], &exp_mem_len, &symbc, &wphase, 0, &i);
                // it's second number case
                } else if (exp_members[0] != NULL && exp_members[1] != NULL && exp_members[2] != NULL) {
                    read_exp_member(is_digit, ch, &exp_members[1], &exp_mem_len, &symbc, &wphase, 2, &i);
                // it's operation case
                } else {
                    read_exp_member(is_operation, ch, &exp_members[2], &exp_mem_len, &symbc, &wphase, 0, &i);
                }
                break;
            ...
            case 2:
                ...
                if (is_ending_sequence && ch == '\n') {
                    ...
                    if (process_int32(exp_members[0], &first_number, &answer) &&
                            process_int32(exp_members[1], &second_number, &answer)) {
                        int64_t result;
